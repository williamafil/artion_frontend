    ...
    <div class="overflow-x-auto mt-6">
      <table class="table-auto border-collapse w-full">
        <thead>
          ...
          >
            <th class="px-4 py-2">訂單號碼</th>
            <th class="px-4 py-2">名稱</th>
            <th class="px-4 py-2">價格</th>
            <th class="px-4 py-2">付款狀態</th>
        ...
        <tbody class="text-sm font-normal text-gray-700">
          ...
          >
            <td class="px-4 py-4">{{ item.order_number }}</td>
            <td class="px-4 py-4">{{ item.title }}</td>
            <td class="px-4 py-4">{{ item.total_price | separator | dollar }}</td>
            <td class="px-4 py-4">
              <span v-if="item.is_paid" class="text-green-500">已付款</span>
              <span v-else class="text-red-500">未付款</span>
            </td>
            <td class="px-4 py-4">
              <div v-if="!item.is_paid" class="text-xs rounded-md p-2 bg-gray-100  hover:bg-yellow-100 cursor-pointer">付款</div>
            </td>
          </tr>
        </tbody>
      </table>
