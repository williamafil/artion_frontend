      ...
      <table class="table-auto border-collapse w-full">
        <thead>
          ...
          >
            ...
            <th class="px-4 py-2 " style="background-color:#f8f8f8">作者</th>
        ...
        <tbody class="text-sm font-normal text-gray-700">
          <tr
            v-for="item in followedAuctions"
            :key="item.id"
            class="hover:bg-gray-100 border-b border-gray-200 py-10"
          >
            <td class="px-4 py-4">{{ item.title }}</td>
            <td class="px-4 py-4">{{ item.author_name }}</td>
            <td class="px-4 py-4">{{ item.start_time | date }}</td>
            <td class="px-4 py-4">{{ item.end_time | date }}</td>
            <td class="px-4 py-4">
              <!-- like_id: {{ likes.filter((x) => x.auction_id === item.id)[0].id }} -->
              <button
                @click="
                  removeHeartHandler(item.id, likes.filter((x) => x.auction_id === item.id)[0].id)
                "
                type="button"
                class="text-xs rounded-md p-2 bg-gray-100  hover:bg-yellow-100"
              >
                取消關注
              </button>
            </td>
...
<script>
import { follows, removeLike } from '@/service/api';
...
export default {
  ...
  created() {
    follows(this.user.id).then((res) => {
      ...
      this.followedAuctions = res.data.data.follows;
      this.likes = res.data.data.likes;
    });
  },
  methods: {
    removeHeartHandler(auctionId, likeId) {
      // console.log('傳進來的auctionId: ', auctionId);
